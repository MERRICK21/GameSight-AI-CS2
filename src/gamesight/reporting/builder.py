...
class EvidenceReportBuilder:
    ...
    def build(self, analysis: AnalysisResult, tracks: list[Track] | None = None) -> MatchReport:
        ...
        personal_kills = analysis.capabilities.get(
            "personal_kills", personal_combat,
        )
        personal_deaths = analysis.capabilities.get(
            "personal_deaths", personal_combat,
        )
        timeline = self._tl_builder.build(analysis, tracks)
        round_reports = [
            self._build_round_report(
                ra, tl_round, personal_combat, personal_kills, personal_deaths,
            )
    ...
    def _build_round_report(
        self, ra: RoundAnalysis, tl_round, personal_combat: bool = True,
        personal_kills: bool = True, personal_deaths: bool = True,
    ) -> RoundReport:
        stats = self._compute_round_stats(
            ra, tl_round, personal_combat, personal_kills, personal_deaths,
        )
        findings = self._build_round_findings(
            ra, stats, personal_combat, personal_kills, personal_deaths,
        )
        return RoundReport(
    ...
    def _compute_round_stats(
        ...
        likely_firefights = 0
        shot_candidate_windows = 0
        damage_candidate_windows = 0
        first_person: dict = {}
        for event in ra.events:
            ...
            if et == EventType.PLAYER_KILL: kills += 1
                ...
                if event.attributes.get("engagement_level") == "likely_firefight":
                    likely_firefights += 1
                if int(event.attributes.get("shot_candidate_count", 0)) > 0:
                    shot_candidate_windows += 1
                if int(event.attributes.get("damage_candidate_count", 0)) > 0:
                    damage_candidate_windows += 1
        ...
        if duration is not None and personal_deaths:
        ...
        return RoundStats(
            ...
            personal_kills_available=personal_kills,
            personal_deaths_available=personal_deaths,
            ...
            likely_firefights=likely_firefights,
            shot_candidate_windows=shot_candidate_windows,
            damage_candidate_windows=damage_candidate_windows,
        )

    def _build_round_findings(
        self, ra: RoundAnalysis, stats: RoundStats,
        personal_combat: bool = True, personal_kills: bool = True,
        personal_deaths: bool = True,
    ) -> list[ReportFinding]:
        ...
        if personal_kills and stats.kills_detected > 0:
        ...
        if personal_deaths and stats.player_died:
            death_events = [e for e in ra.events if e.event_type == EventType.PLAYER_DEATH]
            ...
            findings.append(self._make_finding(f"{self._PREFIX_COMBAT}_deaths",
                ...
                t.t(
                    "report_finding.native_death_detected"
                    if native_status else "report_finding.deaths_detected",
                    n=1,
                ), 0.85, evidence))
        if (personal_kills and personal_deaths and stats.kills_detected == 0
                and not stats.player_died):
        ...
        if stats.likely_firefights > 0:
            ...
            evidence = [
                link for event in fight_events
                for link in self._to_links(event.evidence)
            ]
            findings.append(self._make_finding(
                f"{self._PREFIX_COMBAT}_likely_firefight",
                FindingCategory.COMBAT,
                FindingSeverity.INFO,
                t.t("report_finding.likely_firefight", n=stats.likely_firefights),
                0.76,
                evidence,
            ))
    ...
    def _build_overview(self, analysis, round_reports, timeline) -> MatchOverview:
        ...
        return MatchOverview(
            ...
            analysis_complete=analysis.capabilities.get(
            ...
            personal_kills_available=analysis.capabilities.get(
                "personal_kills",
                analysis.capabilities.get("personal_combat", True),
            ),
            personal_deaths_available=analysis.capabilities.get(
                "personal_deaths",
                analysis.capabilities.get("personal_combat", True),
            ),
            ...
            total_likely_firefights=sum(
                round_report.stats.likely_firefights for round_report in round_reports
            ),
        )
    ...
    def _build_match_findings(self, analysis, overview) -> list[ReportFinding]:
        ...
        if overview.personal_kills_available and overview.total_kills_detected > 0:
        ...
        if overview.personal_deaths_available and overview.total_deaths_detected > 0:
        ...
        if (overview.personal_kills_available and overview.personal_deaths_available
                and overview.total_kills_detected == 0
